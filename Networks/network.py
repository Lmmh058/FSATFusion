...
class MultiSpectralAttentionLayer(torch.nn.Module):
    def __init__(self, channel=16, dct_h=224, dct_w=224, reduction=16, freq_sel_method='top16'):
...
class FAM_Attention(nn.Module):
    def __init__(self, in_channels, out_channels, rate=4):
        super(FAM_Attention, self).__init__()

        self.channel_attention = MultiSpectralAttentionLayer()
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        std_out = torch.std(x_channel, dim=1, keepdim=True, unbiased=False)
        concat = torch.cat([max_out, std_out], dim=1)
        x_spatial = self.sigmoid(self.conv1(concat))
...
class BasicLayer(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.FAM_Attention = FAM_Attention(dim, dim)
    ...
    def forward(self, x, x_size):
        for blk in self.blocks:
            x = blk(x, x_size)
            shortcut = x
            attention_x = self.FAM_Attention(x)
            out_x = shortcut + attention_x
            x = self.bn(out_x)
            x = self.relu(x)
...
class MODEL(nn.Module):
    def __init__(self, img_size=120, patch_size=4, embed_dim=96, num_heads=8, window_size=1, in_channel=2,
                 out_channel=16, output_channel=1,
        ...
        self.FAM_attention = FAM_Attention(out_channel, out_channel)
        self.relu = nn.ReLU(True)
        ...
        self.conv64_16 = nn.Conv2d(64, 16, kernel_size=1, padding=0, stride=1)

    def forward(self, input):
        encode = self.convolutional(input)
        encode_size = (encode.shape[2], encode.shape[3])
        Transformer = self.basicLayer(encode, encode_size)
        out = self.convolutional_out(Transformer)
        return out
